...
void Dock::focusContainer(){
  ...
  if (!IsAnyItemActive() && !IsAnyItemHovered() && g->IO.MouseClicked[0]){
    if (this->root){
      ...
      g->MovingWindow->MoveId = this->root->window->RootWindow->MoveId;
    } else {
      g->MovingWindow = this->window;
      g->MovingWindow->MoveId = this->window->RootWindow->MoveId;
    }
    if (this->currenttab)
      SetActiveID(g->MovingWindow->MoveId, this->currenttab->window->RootWindow);
    else
      SetActiveID(g->MovingWindow->MoveId, this->window->RootWindow);
  }
}
...
Dock *ImGui::Container(const char* label, bool* p_open /*=nullptr*/, ImGuiWindowFlags extra_flags /*= 0*/,
  ...
  if (dd->status == Dock::Status_Docked){
    g->NextWindowData.PosCond = g->NextWindowData.SizeCond = g->NextWindowData.ContentSizeCond = g->NextWindowData.CollapsedCond = 0;
    g->NextWindowData.SizeConstraintCond = g->NextWindowData.FocusCond = false;
    return dd;
  }
...
bool ImGui::BeginDock(const char* label, bool* p_open /*=nullptr*/, ImGuiWindowFlags flags /*= 0*/, 
  ...
  if (dd->status == Dock::Status_Docked || dd->control_window_this_frame){
    ...
    if (dd->status == Dock::Status_Docked) {
      ...
      g->MovingWindow->MoveId = dd->window->RootWindow->MoveId;
      SetActiveID(g->MovingWindow->MoveId, dd->window->RootWindow);
      dd->parent = nullptr;
      dd->root = nullptr;
    } else {
